...
- **Dependencies:** `com.unity.ugui` (2.0.0), `com.unity.timeline` (1.8.12), `com.unity.modules.video` (1.0.0)
...
| Save | `Save_System/` | `SaveManager` persists `ISaveable` objects across several named stores at once — each saveable picks one via `StorageId`, so settings can sit in a local file while progression goes to a remote API. Backends: PlayerPrefs, JSON file, encrypted file, or cloud. A store can declare itself a **mirror** of another (a local cache in front of a cloud backend), which makes saves work offline and get reconciled by `Sync()` with timestamp/playtime/manual conflict resolution. Binary **blobs** — screenshots and other images — ride alongside a slot without going into its JSON. |
